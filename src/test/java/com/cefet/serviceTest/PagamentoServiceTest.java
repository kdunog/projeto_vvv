...
class PagamentoServiceTest {
    ...
    @Test
    @DisplayName("Deve aplicar 40% de desconto infantil parapassageiros entre 2 e 10 anos (RN004)")
...
inv.getArgument(0));
        ...
        assertEquals(0, 
processado.getDesconto().compareTo(BigDecimal.valueOf(40.0)));
        assertEquals(0, processado.getJuros().compareTo(BigDecimal.ZERO));
        assertEquals(0, 
processado.getValorFinal().compareTo(BigDecimal.valueOf(60.0)));
...
inv.getArgument(0));
        ...
        assertEquals(0, 
processado.getJuros().compareTo(BigDecimal.valueOf(5.0)));
        assertEquals(0, 
processado.getValorFinal().compareTo(BigDecimal.valueOf(105.0)));
        assertEquals("CONFIRMADO", processado.getStatus());
    }
    @Test
...
pagamentoService.salvar(pagamento));
        assertEquals("Pagamento deve possuir uma reserva com ID.", 
