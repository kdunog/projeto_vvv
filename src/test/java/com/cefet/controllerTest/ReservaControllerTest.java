...
class ReservaControllerTest {
    ...
    void deveRetornar404ParaReservaInexistente() throws Exception {
        when(reservaService.buscarPorId(99L)).thenThrow(new RuntimeException("Reserva não encontrada!"));
    ...
    void deveCriarReserva() throws Exception {
        ...
        when(reservaService.salvar(any(Reserva.class))).thenAnswer(invocation -> invocation.getArgument(0));
    ...
    void deveRetornar400QuandoRegraFalhar() throws Exception {
        ...
        when(reservaService.salvar(any(Reserva.class))).thenThrow(new RuntimeException("Modal em manutenção não pode receber reservas."));
