...
class PassageiroControllerTest {
	...
	void deveSalvarPassageiro() throws Exception {
		...
		when(service.salvar(any(Passageiro.class))).thenAnswer(invocation -> invocation.getArgument(0));
	...
	void deveRetornar404AoAtualizarParcialmentePassageiroInexistente() throws Exception {
		Passageiro dadosAtualizados = new Passageiro();
		dadosAtualizados.setNome("Ana Silva");
